...
const DEFAULT_GLOW_COLOR = '0, 229, 255';
...
const BentoCardGrid: React.FC<{
  ...
  <div
    className="bento-section grid gap-6 p-6 max-w-6xl mx-auto select-none relative"
    ref={gridRef}
  >
    {children}
  </div>
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-6">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-start relative min-h-[200px] w-full p-6 rounded-2xl border border-solid overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-1 ${
              ...
              return (
                ...
                >
                  <div className="card__content flex flex-col relative h-full">
                    <h3 className={`card__title text-xl font-semibold mb-3 ${textAutoHide ? 'text-clamp-1' : ''}`}>
                      {card.title}
                    </h3>
                    <p className={`card__description text-muted-foreground leading-relaxed ${textAutoHide ? 'text-clamp-2' : ''}`}>
                      {card.description}
                    </p>
                  </div>
                </ParticleCard>
