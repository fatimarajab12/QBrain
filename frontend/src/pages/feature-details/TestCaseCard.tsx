...
import { Play, AlertCircle, CheckCircle2, Edit, Trash2, FileCode, Copy, Check, X, Download } from "lucide-react";
...
const TestCaseCard = ({ testCase, onEdit, onDelete, onStatusUpdate, onPriorityUpdate, isDeleting = false }: TestCaseCardProps) => {
  ...
  const handleOpenGherkinDialog = async () => {
    ...
    try {
      ...
      try {
        const result = await testCaseService.convertToGherkin(testCase._id);
        setGherkinCode(result.gherkin);
      } catch (apiError) {
        // Fallback to local conversion if API fails
        logger.warn('API conversion failed, using local conversion:', apiError);
            ...
            <div className="flex gap-2 flex-wrap items-center">
              {onPriorityUpdate ? (
                <Select
                  ...
                  onValueChange={(value: "high" | "medium" | "low") => onPriorityUpdate(testCase._id, value)}
                >
          ...
          <TabsContent value="results" className="space-y-4 mt-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <Button 
                variant="outline" 
                size="sm"
                onClick={() => onStatusUpdate(testCase._id, "passed")}
              >
              ...
              <Button 
                variant="destructive" 
                size="sm"
                onClick={() => onStatusUpdate(testCase._id, "failed")}
              >
...
export default React.memo(TestCaseCard, (prevProps, nextProps) => {
  return (
    prevProps.testCase._id === nextProps.testCase._id &&
    prevProps.testCase.status === nextProps.testCase.status &&
    prevProps.testCase.priority === nextProps.testCase.priority &&
    prevProps.testCase.title === nextProps.testCase.title &&
