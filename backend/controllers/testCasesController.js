...
export const getAllTestCasesByFeatures = async (req, res, next) => {
  try {
    const { projectId } = req.params;
    ...
    const data = await testCaseService.getAllTestCasesByFeatures(projectId);

    if (!data.features || data.features.length === 0) {
      return next(new NotFoundError("No features found for this project"));
    }

    // Print to console (for debugging)
    console.log("\n=== All Test Cases by Features ===");
    console.log(`Project: ${data.project.name}`);
    console.log(`Total Features: ${data.totalFeatures}`);
    console.log(`Total Test Cases: ${data.totalTestCases}\n`);

    data.features.forEach((featureGroup, index) => {
      console.log(`${index + 1}. Feature: ${featureGroup.feature.name}`);
      console.log(`   Priority: ${featureGroup.feature.priority}, Status: ${featureGroup.feature.status}`);
      console.log(`   Test Cases Count: ${featureGroup.testCasesCount}`);
      
      if (featureGroup.testCases.length > 0) {
        featureGroup.testCases.forEach((tc, tcIndex) => {
          console.log(`   ${tcIndex + 1}. ${tc.title} (${tc.priority}, ${tc.status})`);
        });
      } else {
    ...
    res.status(200).json({
      success: true,
      message: `Found ${data.totalFeatures} features with ${data.totalTestCases} test cases`,
      data: data,
    });
  } catch (error) {
    next(error);
...
export const exportAllTestCasesToExcel = async (req, res, next) => {
  try {
    const { projectId } = req.params;
    ...
    // Get all test cases grouped by features
    const data = await testCaseService.getAllTestCasesByFeatures(projectId);
    ...
    if (data.totalTestCases === 0) {
      return next(new NotFoundError("No test cases found for this project"));
    }

    // Create a new workbook
    const workbook = new ExcelJS.Workbook();
    workbook.creator = "QBrain";
    ...
    summarySheet.columns = [
      ...
      { header: "Total Features", key: "totalFeatures", width: 15 },
    ...
    const testCasesSheet = workbook.addWorksheet("All Test Cases");
    testCasesSheet.columns = [
      { header: "Feature Name", key: "featureName", width: 30 },
      { header: "Feature Priority", key: "featurePriority", width: 18 },
      { header: "Feature Status", key: "featureStatus", width: 18 },
      { header: "Acceptance Criteria", key: "acceptanceCriteria", width: 60 },
      { header: "Test Case Title", key: "testCaseTitle", width: 40 },
      { header: "Test Case Description", key: "testCaseDescription", width: 50 },
    ...
    data.features.forEach((featureGroup) => {
      if (featureGroup.testCases.length === 0) {
        // Add row for feature with no test cases
        testCasesSheet.addRow({
          featureName: featureGroup.feature.name,
          featurePriority: featureGroup.feature.priority,
          featureStatus: featureGroup.feature.status,
          acceptanceCriteria: Array.isArray(featureGroup.feature.acceptanceCriteria) 
            ? featureGroup.feature.acceptanceCriteria.join("; ") 
            : "",
          testCaseTitle: "No test cases",
          testCaseDescription: "",
          steps: "",
          expectedResult: "",
          priority: "",
        ...
        featureGroup.testCases.forEach((testCase) => {
          testCasesSheet.addRow({
            featureName: featureGroup.feature.name,
            featurePriority: featureGroup.feature.priority,
            featureStatus: featureGroup.feature.status,
    ...
    testCasesSheet.autoFilter = {
      from: "A1",
      to: { row: 1, column: testCasesSheet.columnCount },
    };

    // Create features sheet
    const featuresSheet = workbook.addWorksheet("Features Overview");
    featuresSheet.columns = [
      { header: "Feature Name", key: "featureName", width: 40 },
      { header: "Description", key: "description", width: 50 },
      { header: "Priority", key: "priority", width: 15 },
      { header: "Status", key: "status", width: 15 },
      { header: "Test Cases Count", key: "testCasesCount", width: 18 },
      { header: "Acceptance Criteria", key: "acceptanceCriteria", width: 60 },
    ];
    ...
    data.features.forEach((featureGroup) => {
      featuresSheet.addRow({
        featureName: featureGroup.feature.name,
        description: featureGroup.feature.description || "",
        priority: featureGroup.feature.priority,
        status: featureGroup.feature.status,
        testCasesCount: featureGroup.testCasesCount,
        acceptanceCriteria: Array.isArray(featureGroup.feature.acceptanceCriteria)
          ? featureGroup.feature.acceptanceCriteria.join(", ")
          : "",
      });
    ...
    featuresSheet.getRow(1).font = { bold: true, color: { argb: "FFFFFFFF" } };

    // Enable auto filter
    featuresSheet.autoFilter = {
      from: "A1",
      to: { row: 1, column: featuresSheet.columnCount },
    };

    // Set response headers for Excel download
    const fileName = `QBrain_TestCases_${data.project.name.replace(/[^a-zA-Z0-9]/g, "_")}_${Date.now()}.xlsx`;
    res.setHeader(
      "Content-Type",
