...
export const createFeature = async (req, res, next) => {
  try {
    ...
    if (!featureData.name || !featureData.name.trim()) {
      return next(new BadRequestError("Feature name is required and cannot be empty"));
    }

    if (!featureData.projectId || !featureData.projectId.trim()) {
      return next(new BadRequestError("Project ID is required. Please ensure you are creating the feature within a valid project."));
    }

    // Validate projectId format (MongoDB ObjectId)
    if (!mongoose.Types.ObjectId.isValid(featureData.projectId)) {
      return next(new BadRequestError(`Invalid Project ID format: "${featureData.projectId}". Project ID must be a valid MongoDB ObjectId (24 hex characters).`));
    }
    ...
    try {
      ...
      // Handle specific service errors with clear messages
      if (serviceError.message && serviceError.message.includes("already exists")) {
        return next(new BadRequestError(serviceError.message));
      }
      if (serviceError.message && serviceError.message.includes("not found")) {
        return next(new NotFoundError(serviceError.message));
      }
      if (serviceError.message && serviceError.message.includes("Invalid")) {
        return next(new BadRequestError(serviceError.message));
      }
