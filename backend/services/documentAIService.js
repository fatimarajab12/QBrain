...
// Use the general OCR processor (doesn't require creating a custom processor)
// Format: projects/{project}/locations/{location}/processors/{processor_id}
// For general OCR, we can use the OCR_PROCESSOR which is available by default
...
export async function parsePDFWithDocumentAI(filePath) {
  try {
    // First, we need to create or get an OCR processor
    // For simplicity, we'll use the general OCR processor endpoint
    // Note: You may need to create a processor in the console first
    
    // Read the PDF file
    const imageFile = fs.readFileSync(filePath);
    
    // For Document AI, we need to use the REST API or create a processor first
    // Let's use a simpler approach with the OCR processor
    // The processor ID format: projects/{project}/locations/{location}/processors/{processor_id}
    
    // Check if a custom processor ID is provided
    const processorId = process.env.DOCUMENT_AI_PROCESSOR_ID;
    
    if (!processorId) {
      // If no processor ID, we'll need to create one or use the general OCR
      // For now, throw an error asking to create a processor
      throw new Error(
    ...
    console.log(`Processing PDF with Document AI: ${filePath}`);
              ...
              table.bodyRows.forEach((row) => {
                ...
                if (row.cells && row.cells.length > 0) {
                  row.cells.forEach((cell) => {
                    // Extract text from cell layout using textAnchor
    ...
    // Extract forms/entities if available
    const forms = [];
    if (document.entities && document.entities.length > 0) {
      document.entities.forEach((entity) => {
        const entityText = entity.textAnchor?.textSegments
          ?.map(segment => {
            const startIndex = segment.startIndex || 0;
            const endIndex = segment.endIndex || 0;
            return document.text?.substring(startIndex, endIndex) || "";
    ...
    console.error("Document AI parsing error:", error.message);
    throw error;
  }
}
...
async function parsePDFWithFallback(filePath) {
  try {
    ...
    // Create test directory if it doesn't exist to avoid pdf-parse error
    const testDataDir = path.join(process.cwd(), "test", "data");
    if (!fs.existsSync(testDataDir)) {
      fs.mkdirSync(testDataDir, { recursive: true });
    }
    
    // Lazy load pdf-parse only when needed (to avoid test file error on module load)
    let pdfParse;
    try {
    ...
    // Estimate page count (pdf-parse doesn't provide exact page count, but we can estimate)
    // Average page is about 2000-3000 characters, so we estimate based on text length
  ...
  try {
    ...
    try {
      ...
      for (let startPage = 0; startPage < totalPages; startPage += maxPagesPerChunk) {
        ...
        const pages = await chunkDoc.copyPages(pdfDoc, Array.from({ length: chunkPages }, (_, i) => startPage + i));
      ...
      const mergedTables = results.flatMap((r, index) =>
        (r.tables || []).map((table) => ({
          ...table,
