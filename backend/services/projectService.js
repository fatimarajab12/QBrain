...
export async function deleteProject(id) {
  ...
  try {
    logger.info(`Starting comprehensive deletion of project ${projectIdString} and ALL associated data...`);

    // 1. Get all related data before deletion
    const { Feature } = await import("../models/Feature.js");
    const { TestCase } = await import("../models/TestCase.js");
    const { Bug } = await import("../models/Bug.js");
    const { PerformanceMetric } = await import("../models/PerformanceMetric.js");
    ...
    const performanceMetrics = await PerformanceMetric.find({ projectId: projectId });
    
    logger.info(
      `Found: ${features.length} features, ${testCases.length} test cases, ${bugs.length} bugs, ${performanceMetrics.length} performance metric(s)`
    );
    ...
    try {
      logger.info(`Attempting to delete vector documents for project ${projectIdString} from Supabase...`);
      const result = await vectorStore.deleteProject(projectIdString);
      logger.info(`Deleted ${result?.deleted || 0} vector document(s) from Supabase (SRS chunks, features, test cases)`);
    } catch (error) {
      logger.error(`Error deleting vector documents from Supabase:`, error);
      logger.error('Error details:', error.message);
      // Continue with deletion even if vector store deletion fails
      logger.info('Continuing with MongoDB deletion despite Supabase error...');
    }

    // 3. Delete bugs from MongoDB
    const bugsDeleted = await Bug.deleteMany({ projectId: projectId });
    logger.info(`Deleted ${bugsDeleted.deletedCount} bug(s) from MongoDB`);

    // 4. Delete performance metrics from MongoDB
    const perfDeleted = await PerformanceMetric.deleteMany({ projectId: projectId });
    logger.info(`Deleted ${perfDeleted.deletedCount} performance metric(s) from MongoDB`);

    // 5. Delete test cases from MongoDB
    ...
    logger.info(`Deleted ${testCasesDeleted.deletedCount} test case(s) from MongoDB`);

    // 6. Delete features from MongoDB
    const featuresDeleted = await Feature.deleteMany({ projectId: projectId });
    logger.info(`Deleted ${featuresDeleted.deletedCount} feature(s) from MongoDB`);

    // 7. Delete SRS document file from filesystem
    if (project.srsDocument && project.srsDocument.filePath) {
      try {
        if (fs.existsSync(project.srsDocument.filePath)) {
          ...
          logger.info(`Deleted SRS file from filesystem: ${project.srsDocument.filePath}`);
        }
      } catch (error) {
        logger.error(`Error deleting SRS file from filesystem:`, error);
      }
    }

    // 8. Finally, delete the project itself from MongoDB
    await Project.findByIdAndDelete(projectId);
    logger.info(`Deleted project ${projectIdString} from MongoDB`);
    
    logger.info(`Project ${projectIdString} and ALL associated data deleted successfully:`);
    logger.info(
      `   - MongoDB: Project, ${featuresDeleted.deletedCount} features, ${testCasesDeleted.deletedCount} test cases, ${bugsDeleted.deletedCount} bugs, ${perfDeleted.deletedCount} performance metric(s)`
    );
    logger.info(`   - Supabase: All vector documents (SRS chunks, features, test cases)`);
    logger.info(`   - Filesystem: SRS document file`);
  } catch (error) {
    logger.error("Error deleting project and associated data:", error);
    throw error;
  }
}
...
export async function uploadAndProcessSRS(projectId, filePath, fileName) {
  logger.info(`Starting SRS processing for project: ${projectId}`);
  ...
  if (filePath.endsWith(".pdf")) {
    ...
    logger.info("Parsing PDF with Document AI...");
    ...
    const { validateDocumentProcessing } = await import("./documentProcessingValidator.js");
    validateDocumentProcessing(filePath, documentAIResult);
    
    // Log additional extracted data
    if (documentAIResult.tables && documentAIResult.tables.length > 0) {
      logger.info(`Document AI extracted ${documentAIResult.tables.length} tables from the PDF`);
    }
    if (documentAIResult.forms && documentAIResult.forms.length > 0) {
      logger.info(`Document AI extracted ${documentAIResult.forms.length} form fields from the PDF`);
    }
  ...
  // SRS-optimized chunking: smaller chunks for better relevance in requirements documents
  // SRS documents contain many short requirements - larger chunks cause embedding confusion
  // LLM understands better with focused context around each requirement
  const splitter = new RecursiveCharacterTextSplitter({
    chunkSize: 1000, // Optimized for SRS: 800-1200 chars for better requirement separation
    chunkOverlap: 175, // Optimized overlap: 150-200 chars to maintain context continuity
  });

  const chunks = await splitter.splitText(text);
  logger.info(`Split SRS into ${chunks.length} chunks (optimized for cost)`);
  const nContextChunks = calculateDynamicContextChunks(chunks.length);
  logger.info("Using dynamic nContextChunks:", nContextChunks);
  ...
  try {
    ...
    logger.info(`Generated embeddings for all chunks using cost-optimized model`);
  ...
  await project.save();
  logger.info("Project SRS data updated");

  // Users can generate them manually from the UI when needed
  logger.info("SRS processed successfully. Features and test cases can be generated manually from the UI.");
