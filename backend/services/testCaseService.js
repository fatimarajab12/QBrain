...
export async function getAllTestCasesByFeatures(projectId) {
  try {
    const id = validateObjectId(projectId, "Project ID");
    ...
    const features = await Feature.find({ projectId: id })
      .select("_id name description priority status acceptanceCriteria")
      .sort({
        priority: -1,
        createdAt: -1,
      })
    ...
    // Get all test cases for the project (explicitly include isAIGenerated)
    const allTestCases = await TestCase.find({ projectId: id })
      .select("-__v")
      .sort({
        priority: -1,
        createdAt: -1,
    ...
    // Ensure isAIGenerated is properly set for all test cases
    allTestCases.forEach((tc) => {
      if (tc.isAIGenerated === undefined || tc.isAIGenerated === null) {
        tc.isAIGenerated = false;
      }
    });

    // Group test cases by feature
    const featuresWithTestCases = features.map((feature) => {
      const featureTestCases = allTestCases.filter(
        (tc) => tc.featureId && tc.featureId.toString() === feature._id.toString()
      );

      // Clean test case data
      const cleanedTestCases = featureTestCases.map((tc) => {
        const cleaned = { ...tc };
        delete cleaned.__v;
        ...
        if (cleaned.featureId) {
          cleaned.featureId = cleaned.featureId.toString();
        }
        if (cleaned.projectId) {
          cleaned.projectId = cleaned.projectId.toString();
        }
        
        // Explicitly preserve isAIGenerated field (ensure it's a boolean)
        // Handle various truthy values that might come from database
        if (cleaned.isAIGenerated === true || cleaned.isAIGenerated === "true" || cleaned.isAIGenerated === 1) {
          cleaned.isAIGenerated = true;
        } else {
          cleaned.isAIGenerated = false;
        }
    ...
    // Calculate totals
    const totalFeatures = featuresWithTestCases.length;
    ...
    return {
      ...
      totalFeatures,
      totalTestCases,
      features: featuresWithTestCases,
    };
  } catch (error) {
    console.error("Error getting all test cases by features:", error);
    throw error;
  }
}
...
export async function generateTestCasesForFeature(featureId, options = {}) {
  try {
    ...
    // Return generated test cases without saving them - saving will happen during approval
    ...
    for (const testCaseData of generatedTestCases) {
      ...
      // Generate a temporary ID for frontend use (will be replaced when actually saved)
      ...
      testCasesWithMetadata.push(testCaseWithMetadata);
    }

    console.log(`Generated ${testCasesWithMetadata.length} test cases for feature: ${feature.name}`);
...
export async function bulkCreateTestCases(featureId, testCasesData) {
  try {
    ...
    for (const testCaseData of testCasesData) {
      // Ensure isAIGenerated is explicitly set if provided, otherwise default to false
      const isAIGenerated = testCaseData.isAIGenerated !== undefined 
        ? testCaseData.isAIGenerated 
        : (testCaseData.aiGenerationContext ? true : false);
      ...
      // Add to vector database for chatbot support (same logic as individual createTestCase)
      try {
        const projectId = testCase.projectId.toString();
        const featureId = testCase.featureId.toString();
        ...
        console.log(`Added test case ${testCase._id} to vector database (bulk creation)`);
      } catch (vectorError) {
        // Log error but don't fail the creation
        console.error("Error adding test case to vector database (bulk):", vectorError);
      }
