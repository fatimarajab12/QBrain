/**
 * Section Management Module - Handles section grouping and coverage analysis
 */
...
export function groupChunksBySections(chunks) {
  ...
  for (const chunk of chunks) {
    ...
    if (sectionNumbers.length > 0) {
      ...
      sectionNumbers.forEach(sec => sectionsMap[primarySection].allSections.add(sec));
    } else {
...
export function analyzeSectionCoverage(features, groupedChunks) {
  ...
  const allSections = Array.from(new Set([...featureSections, ...chunkSections]));
  const coveragePercentage = allSections.length > 0 
    ? (coveredSections.length / allSections.length) * 100 
    : 0;
  
  // Find missing sections (sections in chunks but not in features)
  const missingSections = Array.from(chunkSections).filter(sec => !featureSections.has(sec));
  ...
  return {
    ...
    recommendations: missingSections.length > 0 
      ? `Consider reviewing sections: ${missingSections.slice(0, 5).join(", ")}${missingSections.length > 5 ? "..." : ""}`
