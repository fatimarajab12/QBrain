/**
 * Prompt Templates Module - Handles adaptive prompt generation
 */
...
- DATA: Data fields, tables, dictionaries, data structures
...
- Data structures and fields (all of them)
...
- Non-functional requirements (even if they seem like comments)
- Every requirement in every section - do not skip any
...
**CRITICAL - EXTRACT EVERYTHING (COMPLETENESS IS KEY):**
- Extract ALL features you find in the SRS text - NO EXCEPTIONS, NO SKIPPING
- Be extremely thorough - don't miss ANY features, even if they seem minor, brief, or appear as comments
- Extract requirements that appear as comments, notes, brief mentions, or in appendices
- Do NOT ignore non-functional requirements, constraints, quality attributes, or assumptions
...
- If a section has multiple requirements, extract EACH ONE as a separate feature
...
- Cover all feature types: functional, data, interfaces, quality, reports, constraints, notifications, workflows
- Remember: A brief requirement is still a requirement - extract it!
- Quality over speed: Take your time to ensure you extract EVERYTHING
- Before finishing, review your list and make sure you didn't miss any requirements
...
export function createTestCasePromptByFeatureType(featureType, matchedSections = []) {
  const sectionContext = matchedSections.length > 0 
...
1. **Performance Tests:**
   - Test response time requirements (e.g., PE-1: 4 seconds)
...
4. **Availability Tests:**
   - Test uptime requirements (e.g., 99.9% availability)
...
1. **Report Content Tests:**
   ...
   - Test report format and layout
   - Test calculations and aggregations
...
`
  ...
  return typeSpecificGuidance[featureType] || typeSpecificGuidance.FUNCTIONAL;
}
