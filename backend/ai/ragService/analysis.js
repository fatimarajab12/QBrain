...
- Acceptance Criteria: ${Array.isArray(feature.acceptanceCriteria) ? feature.acceptanceCriteria.join(", ") : 'N/A'}
`;
        }
      } catch (error) {
    ...
    // Get SRS context if projectId is available
    let srsContext = "";
    if (projectId) {
      try {
        const contextQuery = `${testCase.title} ${testCase.description || ""} ${testCase.expectedResult || ""}`;
...
`;
    ...
    const prompt = PromptTemplate.fromTemplate(`You are an expert QA engineer specializing in Behavior-Driven Development (BDD) and Gherkin syntax.
...
**OUTPUT:**`);
    ...
    const formattedPrompt = await prompt.format({
      ...
      preconditions: Array.isArray(testCase.preconditions) && testCase.preconditions.length > 0
        ? testCase.preconditions.join("\n- ") 
        : "None",
      steps: Array.isArray(testCase.steps) && testCase.steps.length > 0
