...
import { SRS_TYPE_PATTERNS } from "../config/constants.js";
...
export async function detectSRSType(projectId) {
  try {
    ...
    const sampleChunks = await getRAGContext(
      projectId,
      "requirements specification document structure format",
      20
    );
    ...
    for (const [type, pattern] of Object.entries(SRS_TYPE_PATTERNS)) {
      ...
      for (const keyword of pattern.keywords) {
        ...
        const matches =
          (textLower.match(new RegExp(escapedKeyword, "g")) || []).length;
      ...
      const normalizedStructuralScore =
        pattern.structuralPatterns && pattern.structuralPatterns.length > 0
          ? (structuralScore / pattern.structuralPatterns.length) *
            structuralWeight
          : 0;
    ...
    if (sortedTypes.length > 1) {
      ...
      const scoreDifference =
        scores[detectedType].totalScore - secondBestScore;
    ...
    console.log(
      `Detected SRS type: ${result.name} (confidence: ${result.confidence}%)`
    );
